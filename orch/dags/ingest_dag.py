...
def spark_cmd(job):
    ...
    match job:
        ...
        case _:
            raise ValueError("Job inválido. Opciones: silver | silver_model | gold_model | test")
...
with DAG(
    ...
    task_unique_ingest = PythonOperator(
        ...
        python_callable=lambda: call_endpoint_with_logging("ingest_unique", "/ingest/world-bank",
                                                          {"countries":"ARG","indicators":"NY.GDP","start_year":2023,"end_year":2024,"isTest":True}),
        retries=3,
        retry_delay=timedelta(minutes=5)
    )
    ...
    run_annual_ingest = ShortCircuitOperator(
        task_id="run_annual_ingest",
        python_callable=should_run_annual
    )
    ...
    mark_annual = PythonOperator(
        task_id="mark_annual",
        python_callable=mark_annual_done
    )
    ...
    run_annual_ingest >> health_annual >> task_annual_ingest >> [mark_annual, notify_annual_success]

    # open meteo
    check_weekly_ingest = ShortCircuitOperator(
        task_id="check_weekly_ingest",
        python_callable=lambda: Variable.get(WEEKLY_INGEST_FLAG, default_var=None) is None
    )
    ...
    task_weekly_ingest = PythonOperator(
        ...
        python_callable=lambda: call_endpoint_with_logging("ingest_weekly", "/ingest/open-meteo",
                                                          {"country":"ARG","start_date":"2023-10-01","end_date":"2024-10-01","isTest":True}),
        retries=3,
        retry_delay=timedelta(minutes=5)
    )
    ...
    check_weekly_ingest >> health_weekly >> task_weekly_ingest >> [mark_weekly_done, notify_weekly_success]
    ...
    # transform_gate >> start_ec2_task >> run_silver_task >> run_silver_model_task >> run_gold_task >> notify_transform_success >> stop_ec2_task
