...
class GoldKPI06(BaseGoldKPIJob):
    ...
    def _get_safe_water_pct(self) -> DataFrame:
        ...
        if water_safe_df.count() == 0:
            unique_service_levels = [
                row['service_level_key'] for row in wash_df.select("service_level_key").distinct().limit(20).collect()
            ]
            unique_service_types = [
                row['service_type_key'] for row in wash_df.select("service_type_key").distinct().limit(20).collect()
            ]
        # ---------------------------------------
    ...
    def _get_gdp_per_capita(self) -> DataFrame:
        ...
        gdp_df = socio_df.filter(
            # USANDO CLAVE NUMÉRICA DE LA DIMENSIÓN
            F.col("indicator_key") == WDI_INDICATORS["gdp_per_capita_key"]
        ).withColumn(
            ...
            "country_key",
            "year",
            F.col("indicator_value").alias("gdp_per_capita")
        )

        # --- Lógica de Debugging para 0 filas ---
        if gdp_df.count() == 0:
            unique_indicators = [
                row['indicator_key'] for row in socio_df.select("indicator_key").distinct().limit(20).collect()
            ]
        # ---------------------------------------
    ...
    def build(self) -> DataFrame:
        ...
        if base_metrics_df.count() == 0:
            ...
            return self.spark.createDataFrame([], "region_name: string, year: int, n_countries: int, corr_water_vs_gdp: decimal(6,3), corr_abs_value: decimal(6,3), avg_safe_water_pct: decimal(5,2), avg_gdp_per_capita: decimal(18,2), risk_level: string")
        ...
        final_df = country_year_metrics.groupBy("region_name", "year").agg(
            F.count("country_key").alias("n_countries"),
            # Correlación de Pearson
            F.corr("safe_water_pct_total", "gdp_per_capita").alias("corr_water_vs_gdp"),
        ...
        ).filter(F.col("n_countries") >= 2) # Solo regiones con 2+ países para correlación
