...
class GoldKPI04WeightedIndex(BaseGoldKPIJob):
    ...
    def _get_sanitation_coverage(self) -> DataFrame:
        ...
        """
        self.log("Calculando % saneamiento básico/seguro total país/año y transformando a riesgo...")
        ...
        sanitation_pct_df = sanitation_safe_df.groupBy(
            "country_key", "year"
        ).agg(
        ...
        sanitation_risk_df = sanitation_pct_df.withColumn(
            "sanitation_risk_pct",
            F.lit(100.0) - F.col("sanitation_coverage_pct")
        )

        return sanitation_risk_df.select("country_key", "year", "sanitation_coverage_pct", "sanitation_risk_pct")
    ...
    def _get_socioeconomic_metrics(self) -> DataFrame:
        ...
        dim_indicator = self.read_silver_table("indicator").select("indicator_key", "indicator_code")

        joined_df = socio_df.join(
            dim_indicator, on="indicator_key", how="inner"
        )
        ...
        filtered_df = joined_df.filter(
            ...
            "country_key", "year", "indicator_code", "value"
        )

        pivot_df = filtered_df.groupBy("country_key", "year").pivot(
            "indicator_code", list(WDI_INDICATORS.values())
        ).agg(F.first("value"))
        ...
        pivot_df = pivot_df.withColumns({
            "child_mortality_rate": F.col("child_mortality_rate").cast("decimal(8,3)"),
    ...
    def _normalize_and_weight(self, df: DataFrame) -> DataFrame:
        ...
        for col_name, limits in SCALING_LIMITS.items():
            ...
            if range_val > 0:
                ...
                normalization_expr = F.when(
                    F.col(col_name).isNull(), F.lit(None) # Mantener NULL si falta dato
                ).when(
                    F.col(col_name) >= F.lit(max_val), F.lit(1.0) # Cap a 1.0 si >= Max
                ).when(
                    F.col(col_name) <= F.lit(min_val), F.lit(0.0) # Cap a 0.0 si <= Min
                ).otherwise(
                ...
                normalization_expr = F.when(
                    ...
                    F.col(col_name).isNotNull(), F.lit(1.0)
        ...
        for col_name, weight in WEIGHTS.items():
            ...
            weighted_term = F.coalesce(F.col(f"{col_name}_norm"), F.lit(0.0)) * F.lit(weight)
    ...
    def build(self) -> DataFrame:
        ...
        final_df = final_df.withColumn(
            ...
            .when(F.col("health_risk_index") > INDEX_UMBRALES["amarillo_max_index"], "Rojo")
            .when(F.col("health_risk_index") > INDEX_UMBRALES["verde_max_index"], "Amarillo")
            .otherwise("Verde")
        )
