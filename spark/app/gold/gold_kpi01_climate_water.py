...
class GoldKPI01ClimateWater(BaseGoldKPIJob):
    ...
    def build(self) -> DataFrame:
        ...
        climate_with_year = climate_raw.withColumn("year", self._year_from_date_key("date_key"))
        wash_with_year = wash_raw.withColumn("year", self._year_from_date_key("date_key"))

        # ===============================================
        # 3) Filtrar WASH a agua segura y países MEX/ARG
        # ===============================================
        self.log("Aplicando filtros de agua segura y usando Broadcast para enriquecimiento...")
        ...
        if safe_count == 0:
            ...
            schema = ("country_key INT, country_name STRING, " "residence_type_key INT, residence_type_desc STRING, " "year INT, " "precip_total_mm_year DOUBLE, delta_precip_mm DOUBLE, " "safe_water_pct DOUBLE, delta_safe_water_pp DOUBLE, " "start_year INT, end_year INT, years_observed BIGINT, " "corr_precip_vs_water DOUBLE, corr_abs_value DOUBLE, " "risk_level STRING, impact_direction STRING")
        ...
        wash_safe_yearly = (
            ...
            .groupBy(
                "country_key", "country_iso3", "country_name",
                "residence_type_key", "residence_type_desc", "year",
            )
        ...
        stats = wash_safe_yearly.agg(F.min("year").alias("min_year"), F.max("year").alias("max_year")).collect()[0]
        ...
        climate_for_countries = (
            ...
            .join(B_country_df, on="country_key", how="left")
            .filter(F.col("country_iso3").isin(self.COUNTRIES_ISO3_FILTER))
            .filter((F.col("year") >= F.lit(min_year)) & (F.col("year") <= F.lit(max_year)))
            .select(
                "country_key", "country_iso3", "country_name", "year",
                F.col("precip_total_mm_year").cast("double").alias("precip_total_mm_year"),
            )
        )
        ...
        series_df = (
            ...
            .select(
                "country_key", "country_name", "country_iso3", "residence_type_key",
                "residence_type_desc", "year", "precip_total_mm_year", "safe_water_pct",
            )
        )
        
        if series_df.rdd.isEmpty():
            self.log("Después de unir clima anual con agua segura, no quedaron filas. Devolviendo DataFrame vacío.")
            schema = ("country_key INT, country_name STRING, " "residence_type_key INT, residence_type_desc STRING, " "year INT, " "precip_total_mm_year DOUBLE, delta_precip_mm DOUBLE, " "safe_water_pct DOUBLE, delta_safe_water_pp DOUBLE, " "start_year INT, end_year INT, years_observed BIGINT, " "corr_precip_vs_water DOUBLE, corr_abs_value DOUBLE, " "risk_level STRING, impact_direction STRING")
            return self.spark.createDataFrame([], schema)
        ...
        w = Window.partitionBy(
            "country_key", "country_name", "residence_type_key", "residence_type_desc",
        ).orderBy("year")
        ...
        deltas = (
            ...
            .withColumn("delta_precip_mm", F.col("precip_total_mm_year") - F.col("prev_precip"))
            .withColumn("delta_safe_water_pp", F.col("safe_water_pct") - F.col("prev_safe"))
        ...
        agg_corr = (
            ...
            .agg(
                ...
                F.corr("delta_precip_mm", "delta_safe_water_pp").alias("corr_precip_vs_water"),
            )
        )

        # Lógica de Semáforo y Dirección 
        agg_corr = agg_corr.withColumn("corr_abs_value", F.abs(F.col("corr_precip_vs_water")))
        agg_corr = (
            agg_corr
        ...
        final_df = (
            ...
            .join(B_agg_corr, on=group_cols, how="left")
            .select(
                "country_key", "country_name", "residence_type_key", "residence_type_desc",
                "year", "precip_total_mm_year", "delta_precip_mm",
                "safe_water_pct", "delta_safe_water_pp",
                "start_year", "end_year", "years_observed",
                "corr_precip_vs_water", "corr_abs_value",
                "risk_level", "impact_direction",
            )
        )
