...
class GoldKPI05(BaseGoldKPIJob):
    ...
    def build(self) -> DataFrame:
        self.log("Calculando % agua segura (básico/seguro) por país/año y residencia...")
        ...
        water_pct_df = water_safe_df.groupBy(
            "country_key", "year", "residence_type_clean").agg(
                F.sum("coverage_pct_num").alias("safe_water_pct")
        )

        # ====================================================================
        self.log(f"PUNTO DE CONTROL: Filas agregadas (country/year/residence) antes del pivot: {water_pct_df.count()}")
        # ====================================================================

        # 6. Pivotar usando la columna limpia y los valores exactos 'urban' y 'rural'
        pivoted_df = water_pct_df.groupBy("country_key", "year").pivot(
            "residence_type_clean", ["urban", "rural"]
        ...
        final_df = pivoted_df.withColumn(
            "gap_urban_rural_pp",
            F.coalesce(F.col("water_urban_pct"), F.lit(0)) - F.coalesce(F.col("water_rural_pct"), F.lit(0))
        )

        # 8. Unir con dim_country
        final_df = final_df.join(
            dim_country, on="country_key", how="inner"
        ).na.drop(subset=["water_urban_pct", "water_rural_pct"], how='all') # Eliminar si ambos son NULL
        ...
        final_df = final_df.withColumn(
            "risk_level",
            F.when(
                F.abs(F.col("gap_urban_rural_pp")) >= UMBRALES["amarillo_max_abs_gap"], "Rojo"
            )
            .when(F.abs(F.col("gap_urban_rural_pp")) >= UMBRALES["verde_max_abs_gap"], "Amarillo")
            .otherwise("Verde")
        )
