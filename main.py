...
from utils.wb import fetch_all_world_bank_data, fetch_world_bank_page, wb_sanitize
...
LatamCountry = Enum(
    "LatamCountry",
    {country: country for country in LATAM_COUNTRIES}
)
...
def upload_to_s3(data_bytes, key, content_type='application/octet-stream', metadata=None):
    """Sube datos binarios a S3 con soporte de metadata."""
    try:
...
def ingest_world_bank_data(
    ...
    """
    Ingesta de World Bank con países, indicadores y fecha como parámetros.
    """

    if end_year < start_year:
        raise HTTPException(status_code=400, detail="end_year no puede ser anterior a start_year")
    ...
    for country in country_list:
        ...
        for year in country_df["date"].unique():
            ...
            s3_path = upload_to_s3(
                ...
                metadata={"country": country, "source": "world-bank", "layer": "bronze"}
            )
    ...
    # Cargar a S3
    # file_key = f"{S3_BRONZE_PREFIX}world_bank/world_bank_data_{date.today().isoformat()}.csv"
    # csv_buffer = io.StringIO()
    # df.to_csv(csv_buffer, index=False)
    
    # s3_path = upload_to_s3(csv_buffer.getvalue().encode('utf-8'), file_key, content_type='text/csv')
...
def ingest_open_meteo_simple(
    ...
    """
    Ingesta de OM con varios parametros.
    ...
    try:
        ...
        for (year, month), group_df in df.groupby(["year", "month"]):
            ...
            s3_path = upload_to_s3(
                ...
                file_key,
                content_type='application/octet-stream',
                metadata={"country": country, "source": "open-meteo", "layer": "bronze"}
            )
            uploaded_files.append(s3_path)
        ...
        raise HTTPException(status_code=500, detail=f"Error procesando la respuesta de Open-Meteo: {e}")
    except Exception as e:
...
semaphore = asyncio.Semaphore(5)  # máximo 5 requests simultáneas
...
async def ingest_open_meteo(
    ...
    """
    Ingesta datos climáticos diarios de Open-Meteo para todas las provincias de Argentina.
    """

    async with httpx.AsyncClient() as client:
        tasks = [
            fetch_province(client, prov, info["latitude"], info["longitude"], start_date, end_date, "UTC" if timezone_utc else COUNTRY_TIMEZONE.get(country, "UTC"))
    ...
    # Convertir la columna 'date' a datetime y extraer año/mes
    df["date"] = pd.to_datetime(df["date"])
    df["year"] = df["date"].dt.year
    df["month"] = df["date"].dt.month
