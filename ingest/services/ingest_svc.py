...
def ingest_world_bank_service(
    countries: str, indicators: str, start_year: int, end_year: int, isTest: bool
):
    if isTest:
        return {
            "status": "success",
...
async def ingest_open_meteo_service(
    country, start_date: date, end_date: date, timezone_utc: bool, isTest: bool
):
    ...
    for prov in df["province"].unique():
        ...
        for year in pdf["year"].unique():
            for month in pdf[pdf["year"] == year]["month"].unique():
                ...
                key = (
                    f"{S3_BRONZE_PREFIX}/open_meteo/{country.value}/province={prov}/"
